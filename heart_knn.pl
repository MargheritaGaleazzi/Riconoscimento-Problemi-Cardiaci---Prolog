...
classifica_knn(Istanza,K,Class) :-
...
valuta([malato/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica_knn(Oggetto,51,malato), !,      % prevede correttamente la presenza di problemi cardiaci
	VNA1 is VNA + 1,
	valuta(Coda,VN,VNA1,VP,VPA,FN,FNA,FP,FPA,NC,NCA).
valuta([sano/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica_knn(Oggetto,51,sano), !, % prevede correttamente l'assenza di problemi cardiaci
	VPA1 is VPA + 1,
	valuta(Coda,VN,VNA,VP,VPA1,FN,FNA,FP,FPA,NC,NCA).
valuta([sano/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica_knn(Oggetto,51,malato), !,      % prevede erroneamente la presenza di problemi cardiaci
	FNA1 is FNA + 1,
	valuta(Coda,VN,VNA,VP,VPA,FN,FNA1,FP,FPA,NC,NCA).
valuta([malato/Oggetto|Coda],VN,VNA,VP,VPA,FN,FNA,FP,FPA,NC,NCA) :-
	classifica_knn(Oggetto,51,sano), !, % prevede erroneamente l'assenza di problemi cardiaci
	FPA1 is FPA + 1,
